...
import com.example.combotoolseller.rootServices.QualitypatchdesignsService;
...
public class QualitypatchdesignsImplement extends BaseAll implements QualitypatchdesignsService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        ...
        Elements imgElements = doc.select("button.thumbnail img.loading__animation");
        int startLink = 0;
    ...
    public String getLinkProductByPage(WebDriver driver, String baseFolder, String limit) {
        ...
        while (checkQuantityCard < 10) {
            ...
            Elements productLinks = docLinkProduct.select("div.card div.card__content div.card__information p.card__heading a.full-unstyled-link");
            List<String> listLink = getListLinkProductComponent(productLinks,"https://qualitypatchdesigns.com");
            ...
            if(checkCloneModal == false) {
                boolean checkCloneSale = getClickAction(driver,"button[aria-label='Close']");
