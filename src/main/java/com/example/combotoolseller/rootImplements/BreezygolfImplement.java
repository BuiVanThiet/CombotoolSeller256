...
import com.example.combotoolseller.rootServices.BreezygolfService;
...
public class BreezygolfImplement extends BaseAll implements BreezygolfService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        String nameProduct = getNameProduct(driver,"h1.product_name");
        Thread.sleep(2000);
        ...
        if(nameProduct == null) {
            ...
            wait.until(ExpectedConditions.presenceOfElementLocated(By.tagName("h1.product_name")));
            nameProduct = getNameProduct(driver,"h1.product_name");
            if (nameProduct == null) {
        ...
        Elements imgElements = doc.select("div.product_section div.image-column div.flexslider ol.flex-control-nav li img");
        int startLink = 0;
        for (String url: getListImageComponent(driver,imgElements,"_\\d+x\\d+", "")) {
            if (!limit.equals("")) {
    ...
    public String getLinkProductByPage(WebDriver driver, String baseFolder, String limit) {
        ...
        while (checkQuantityCard < 10) {
            ...
            Elements productLinks = docLinkProduct.select("div.products div.product div.product-card div.info a");
            List<String> listLink = getListLinkProductComponent(productLinks,"https://breezygolf.com");
