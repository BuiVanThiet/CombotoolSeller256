...
public class WalmartImplement extends BaseAll implements WalmartService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        String nameProduct = getNameProduct(driver,"section h1");
        Thread.sleep(2000);
        ...
        if(nameProduct == null) {
            ...
            wait.until(ExpectedConditions.presenceOfElementLocated(By.tagName("section h1")));
            nameProduct = getNameProduct(driver,"section h1");
            if (nameProduct == null) {
        ...
        clickLastElement(driver,"div.overflow-hidden button.pa0.ma0.bg-white.db.pointer.bn.b--transparent.w-100");
        boolean checkDialog = waitForElementPresence(driver,"div.border-box.flex.h-100.w-100.bg-white.overflow-y-scroll.self-center.br4.mb4.mw1660[role='dialog']");
        Elements imgElements = null;
        Document doc = Jsoup.parse(driver.getPageSource());
        System.out.println("checkDialog: "+checkDialog);
        if(checkDialog == true) {
            imgElements = doc.select("div.border-box.flex.h-100.w-100.bg-white.overflow-y-scroll.self-center.br4.mb4.mw1660[role='dialog'] div.relative div.container.overflow-y-hidden.mv3 img");
        } else {
            imgElements = doc.select("div.overflow-hidden button img");
        }
