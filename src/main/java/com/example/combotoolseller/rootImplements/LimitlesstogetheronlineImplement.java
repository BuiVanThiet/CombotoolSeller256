...
import com.example.combotoolseller.rootServices.LimitlesstogetheronlineService;
...
public class LimitlesstogetheronlineImplement extends BaseAll implements LimitlesstogetheronlineService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        ...
        Elements imgElements = doc.select("ul.thumbnail-list li.thumbnail-list__item button.thumbnail img");
        int startLink = 0;
        for (String url: getListImageComponent(driver,imgElements,"\\&height=.*", "")) {
            if (!limit.equals("")) {
    ...
    public String getLinkProductByPage(WebDriver driver, String baseFolder, String limit) {
        ...
        while (checkQuantityCard < 10) {
            ...
            Elements productLinks = docLinkProduct.select("ul#product-grid li.grid__item div.card-wrapper div.card div.card__inner div.card__content div.card__information h3.card__heading a.full-unstyled-link");
            List<String> listLink = getListLinkProductComponent(productLinks,"https://limitlesstogetheronline.com");
