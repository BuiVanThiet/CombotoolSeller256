...
import com.example.combotoolseller.rootServices.BogeybrosService;
...
public class BogeybrosImplement extends BaseAll implements BogeybrosService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        ...
        Elements imgElements = doc.select("product-gallery.product-gallery scroll-shadow.product-gallery__thumbnail-list-wrapper page-dots.product-gallery__thumbnail-list button.product-gallery__thumbnail img.object-contain");
        if (imgElements.size() <= 0) {
            imgElements = doc.select("product-gallery.product-gallery div.product-gallery__ar-wrapper div.product-gallery__media-list-wrapper media-carousel.product-gallery__media-list div.product-gallery__media img");
        }
        int startLink = 0;
    ...
    public String getLinkProductByPage(WebDriver driver, String baseFolder, String limit) {
        ...
        while (checkQuantityCard < 10) {
            ...
            Elements productLinks = docLinkProduct.select("div.v-stack.gap-6 reveal-items product-list.product-list product-card.product-card.product-card--blends.product-card--show-secondary-media.bg-custom.text-custom div.product__inner div.product-card__info div.v-stack.gap-0\\.5.w-full div.rating-with-text span.product-card__title a.bold");
            List<String> listLink = getListLinkProductComponent(productLinks,"https://bogeybros.co");
            ...
            if(checkCloneModal == false) {
                boolean checkCloneSale = getClickAction(driver,"button.needsclick[aria-label='Close dialog']");
