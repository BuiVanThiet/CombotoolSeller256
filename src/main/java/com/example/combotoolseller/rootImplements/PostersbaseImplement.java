...
import com.example.combotoolseller.rootServices.PostersbaseService;
...
public class PostersbaseImplement extends BaseAll implements PostersbaseService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        ...
        Elements imgElements = doc.select("div.pwzrjss1 div.pwzrjss2 div.PwzrGrid-root img");
        if (imgElements.size() <= 0) {
            imgElements = doc.select("img.pwzrjss586");
        }
        int startLink = 0;
