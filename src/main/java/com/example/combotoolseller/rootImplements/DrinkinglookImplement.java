...
import com.example.combotoolseller.rootServices.DrinkinglookService;
...
public class DrinkinglookImplement extends BaseAll implements DrinkinglookService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        ...
        Elements imgElements = doc.select("div.product-details div.product-detail-gallery div.product-detail-gallery-inner div.product-detail-thumbs-container spz-selector.product-detail-thumb-images div div.product-detail-thumbs-item spz-img.cursor-pointer img");
        if (imgElements.size() <= 0) {
            imgElements = doc.select("spz-zoom.zoom-out-image.i-spzhtml-layout-container spz-img.i-spzhtml-layout-responsive img.i-spzhtml-fill-content.i-spzhtml-replaced-content");
        }
        int startLink = 0;
        for (String url: getListImageComponent(driver,imgElements,"_180x", "")) {
            if (!limit.equals("")) {
    ...
    public String getLinkProductByPage(WebDriver driver, String baseFolder, String limit) {
        ...
        while (checkQuantityCard < 10) {
            ...
            Elements productLinks = docLinkProduct.select("div.search-result-content spz-list.search-list div.shoplazza-product-snippet a.product-snippet__price-wrapper");
