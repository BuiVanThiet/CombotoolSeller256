...
import com.example.combotoolseller.rootServices.InprintwetrustService;
...
public class InprintwetrustImplement extends BaseAll implements InprintwetrustService {
    ...
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        String nameProduct = getNameProduct(driver,"h1.product-info__title");
        Thread.sleep(2000);
        ...
        if(nameProduct == null) {
            ...
            wait.until(ExpectedConditions.presenceOfElementLocated(By.tagName("h1.product-info__title")));
            nameProduct = getNameProduct(driver,"h1.product-info__title");
            if (nameProduct == null) {
        ...
        Elements imgElements = doc.select("product-gallery.product-gallery img");
        int startLink = 0;
...
//            break;
        ...
        if(urlImage.size() <= 0) {
            System.out.println("Không có amh");
            return "False";
        }
    ...
    public String getLinkProductByPage(WebDriver driver, String baseFolder, String limit) {
        ...
        while (checkQuantityCard < 10) {
            ...
            Elements productLinks = docLinkProduct.select("span.product-card__title a");
            List<String> listLink = getListLinkProductComponent(productLinks,"https://inprintwetrust.co");
            ...
            boolean checkClickNext = clickNextPage(driver,"a.pagination__item[rel='next']");
