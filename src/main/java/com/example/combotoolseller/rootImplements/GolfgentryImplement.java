...
import com.example.combotoolseller.rootServices.GolfgentryService;
...
public class GolfgentryImplement extends BaseAll implements GolfgentryService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        String nameProduct = getNameProduct(driver,"h2.product-title");
        Thread.sleep(2000);
        ...
        if(nameProduct == null) {
            ...
            wait.until(ExpectedConditions.presenceOfElementLocated(By.tagName("h2.product-title")));
            nameProduct = getNameProduct(driver,"h2.product-title");
            if (nameProduct == null) {
        ...
        Elements imgElements = doc.select("safe-sticky product-gallery-navigation button.product-gallery__thumbnail img");
        int startLink = 0;
...
//            break;
    ...
    public String getLinkProductByPage(WebDriver driver, String baseFolder, String limit) {
        ...
        while (checkQuantityCard < 10) {
            ...
            Elements productLinks = docLinkProduct.select("div.collection__main product-list product-card div.product-card__figure a.product-card__media");
            List<String> listLink = getListLinkProductComponent(productLinks,"https://golfgentry.com");
            ...
            boolean checkClickNext = clickNextPage(driver,"div.collection__main nav.pagination a.pagination__link[rel='next']");
