...
import com.example.combotoolseller.rootServices.IdeasmvdService;
...
public class IdeasmvdImplement extends BaseAll implements IdeasmvdService {
    @Override
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        ...
        Elements imgElements = doc.select("div.slick-track div.la-thumb img");
        int checkImage = 0;
        int startLink = 0;
        for (String url: getListImageComponent(driver,imgElements,"-250x279", "")) {
            if (!limit.equals("")) {
