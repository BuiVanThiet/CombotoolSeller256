...
import com.example.combotoolseller.rootServices.MiashoopService;
...
public class MiashoopImplement extends BaseAll implements MiashoopService {
    ...
    public String getDowloadImage(WebDriver driver, String linkRoot, String baseFolder, int index, String limit) throws IOException, InterruptedException {
        String nameProduct = getNameProduct(driver,"h2.product-info__header_title");
        Thread.sleep(2000);
        ...
        if(nameProduct == null) {
            ...
            wait.until(ExpectedConditions.presenceOfElementLocated(By.tagName("h2.product-info__header_title")));
            nameProduct = getNameProduct(driver,"h2.product-info__header_title");
            if (nameProduct == null) {
        ...
        Elements imgElements = doc.select("div.product-image__thumbs-item div.product-image__thumb img");
...
//        }
        int startLink = 0;
        for (String url: getListImageComponent(driver,imgElements,"_180x\\.(jpg|webp|png|jpeg)\\s*\\d+w", "_3000x.jpeg")) {
            if (!limit.equals("")) {
